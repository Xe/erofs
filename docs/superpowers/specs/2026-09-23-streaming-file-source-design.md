...
v0.7.0 released the flat-file part of this spec (`AddFileFunc` and the lazy
`AddFromFS`). Release the compressed part (the spool, `WithSpoolDir`, and
compression from a reader) as v0.8.0. The API only grows, but a compressed
build now writes a temp file, so a minor version is correct.

## The consumer change in objgit

After v0.8.0, objgit changes `snapshot.Ensure`:
